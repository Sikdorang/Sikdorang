...
import { recommedTypeListLight } from '@/constants/recommend';
...
export default function LightRecommendPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      for (const type of recommedTypeListLight) {
          ...
          <div className="w-full">
            <table className="w-full table-auto border-separate border-spacing-0 overflow-hidden rounded-xl">
              ...
              <tbody className="text-mb-3 rounded-t-none rounded-bl-xl rounded-br-xl border border-t-0 text-gray-700">
                {recommedTypeListLight.map((item, idx) => {
                  ...
                  return (
                    <RecommendTableRow
                      key={item.id}
                      item={item}
                      isLastRow={idx === recommedTypeListLight.length - 1}
                      updateRecommendationTypeMenus={(
